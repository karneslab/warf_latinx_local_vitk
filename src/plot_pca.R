#### This script creates PCA plots from eigenvec file 
#### heidi steiner
#### heidiesteiner@email.arizona.edu
#### Jan 30 2022
#### Last updated: 2022-03-07

#### maybe fix ? 


#### load libraries
library(tidyverse)
library(cowplot)

#### load data
pcas = read_table("results/datasets/methods/data_snpsonly_clean_chrpos_nodups.hgdp1000ghg19_pos.eigenvec",
                  col_names = F)

pr_pcas = read_table("results/datasets/methods/pr_data_snpsonly_clean_chrpos_nodups.hgdp1000ghg19_pos.eigenvec",
                     col_names = F)


#### load 1000genomes data (all pops)
phase3 <- read_table("data/1000GP_Phase3.sample")

table(phase3$POP)


#### select just the reference populations used here
phase3_ref = phase3 %>%  
  filter(POP %in% c("IBS", 
                    "YRI",
                    "CHB")) %>%  #### do I want CHB? 
  arrange(POP)

table(phase3_ref$POP)

rm(phase3)

#### load references
hgdp_1000g <- read_table("results/datasets/methods/hgdp1000ghg19.clean.fam", col_names = F) %>% 
  unite(IID, X1:X2, sep = "_",remove = F) %>% 
  select(IID, X1 = X2)


#### load IDs used in our analysis
hgdp_ref <- read_table("data/hgdp.keep", col_names = F) %>% 
  select(id = X1) %>% 
  mutate(pop = "NAT")

#### join reference data 
ref = phase3_ref %>% 
  select(X1 = ID, POP) %>% 
  full_join(hgdp_ref %>% select(X1 = id, POP=pop)) %>% 
  select(X1, POP) %>% 
  inner_join(hgdp_1000g)

rm(phase3_ref, hgdp_ref, hgdp_1000g)

#### check on NAs
sum(is.na(ref$POP))

table(ref$POP)

#### join PCAs with IDs
dat_long = pcas %>% 
  rename(IID = X2) %>% 
  left_join(ref, by = "IID") %>% 
  select(IID, POP, X3:X12) %>% 
  pivot_longer(cols = starts_with("X"), names_to = "pca") %>% 
  mutate(pca = gsub("X", "", pca),
         pca = as.numeric(pca)-2,
         pca = paste("PC", pca, sep = ""),
         study= "Tucson (n=142)",
         pop = if_else(grepl("warf", IID, ignore.case = T), "study", POP),
         pop = factor(pop, labels = c("Han Chinese (n=103)", "Iberian (n=107)", "Native American (n=46)", "Yoruban (n=108)", "Tucson (n=142)"), levels = c("CHB", "IBS", "NAT", "YRI","study"))) %>% 
  filter(!is.na(pop)) 

# rm(pcas, ref)

pr_dat = pr_pcas%>% 
  rename(IID = X2) %>% 
  left_join(ref, by = "IID") %>% 
  select(IID, POP, X3:X12) %>% 
  pivot_longer(cols = starts_with("X"), names_to = "pca") %>% 
  mutate(pca = gsub("X", "", pca),
         pca = as.numeric(pca)-2,
         pca = paste("PC", pca, sep = ""),
         study = "San Juan (n = 96)",
         pop = if_else(grepl("_R0", IID, ignore.case = T), "study", POP), # this leaves in some others! 
         pop = factor(pop, 
                      labels = c("Han Chinese (n=103)", "Iberian (n=107)", "Native American (n=46)", "Yoruban (n=108)", "San Juan (n=96)"), 
                      levels = c("CHB", "IBS", "NAT", "YRI","study"))) %>% 
  filter(!is.na(pop)) 
  

merge = dat_long %>% 
  full_join(pr_dat)

#### plot 

#### PCA figure for manuscipt
merge %>% 
  pivot_wider(names_from = "pca", values_from = "value") %>% 
  ggplot(aes(x = PC1, y = PC2, color = pop)) + 
  geom_point() +
  scale_color_manual(values = c("red", "#007FFF", "yellow","purple", "gray40", "gray70")) + 
  theme_minimal() +
  labs(color = "Population")+
  theme(legend.position = "bottom")



#dev.off()
ggsave("results/plots/pca.png", last_plot(), width = 6,height = 5)  

