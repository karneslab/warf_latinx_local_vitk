#### clean and combine pre/post impute GWAS GGCX variants with previously recorded taqman/dmet/luminex variants
...
#### rs12714145  GRCh37.p13 chr 2	85787341 C>T
####             GRCh38.p13 chr 2	85560218 C>A

#### 2:85787341-C-T in deliverable_JK-5028-Karnes_BestPerformingSNPs.bim
#### chr2:85560218:C:T  in phased_autosomes_lifted.bim 

#### plink1.9 --bfile ../data/imputed/topmed/phased_autosomes_lifted --snp chr2:85560218:C:T --recode --out ggcx_rs12714145_imputed_tucson
#### plink1.9 --bfile ../data/rawdata/data --d : --snp 2:85787341-C-T --recode --out ggcx_rs12714145_raw_tucson


#### load raw GWAS data 
raw = read_table("results/datasets/ggcx_rs12714145_raw_tucson.ped",
                 col_names = F) %>% 
  select(rawid = X2,
         snp1 = X7, 
         snp2 = X8) %>% 
...
imputed = read_table("results/datasets/ggcx_rs12714145_imputed_tucson.ped",
                     col_names = F) %>% 
  select(id = X2,
         snp1 = X7, 
         snp2 = X8)%>% 
...
write_delim(gene, "results/datasets/tucson_ggcx.tsv", delim = "\t")
...
#### load imputed GWAS data 
imputed = read_table("results/datasets/ggcx_rs12714145_imputed_sj.ped",
                     col_names = F) %>% 
  select(id = X2,
         snp1 = X7, 
         snp2 = X8)%>% 
...
write_delim(vip, "results/datasets/sj_ggcx.tsv", delim = "\t")
