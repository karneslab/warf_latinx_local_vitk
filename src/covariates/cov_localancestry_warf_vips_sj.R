...
#### R script to be run in the server/HPC for the PR cohort
...
#### set paths
path_to_bed_files = "~/Downloads/rfmix/"
name_out_file = "la_warf_vips_K2_pr"
...
A$iid_name<-gsub("0_0_0_0_", "", A$iid)
A$iid_name<- sub(".*?_", "", A$iid_name)
...
B$iid_name<-gsub("0_0_0_0_", "", B$iid)
...
cyp_a <- A %>% 
  ...
  select(iid_name,  CYP2C9_pop1 = gene_pop1) %>% 
  mutate(
         rawid = gsub(".A.bed", "", iid_name, perl = T)) %>% 
  select(-iid_name)
...
cyp_b <- B %>% 
  filter(chr ==10 )%>%
  filter(begin <= 96698415 & end >= 96749848) %>% 
  select(iid_name, CYP2C9_pop2 = gene_pop2) %>% 
  mutate(rawid = gsub(".B.bed", "", iid_name, perl = T)) %>% 
  select(-iid_name)
...
vkor_a <- A %>% 
  ...
  select(iid_name,  VKORC1_pop1 = gene_pop1) %>% 
  mutate(rawid = gsub(".A.bed", "", iid_name, perl = T)) %>% 
  select(-iid_name)
...
vkor_b <- B %>% 
  filter(chr ==16 )%>%
  filter(begin <= 31102175 & end >= 31106118)%>% 
  select(iid_name,  VKORC1_pop2 = gene_pop2) %>% 
  mutate(rawid = gsub(".B.bed", "", iid_name, perl = T)) %>% 
  select(-iid_name)
...
ggcx_a <- A %>% 
  ...
  select(iid_name,  GGCX_pop1 = gene_pop1) %>% 
  mutate(rawid = gsub(".A.bed", "", iid_name, perl = T)) %>% 
  select(-iid_name)
...
ggcx_b <- B %>% 
  filter(chr ==2 )%>%
  filter(begin <= 85771843 & end >= 85788616) %>% 
  select(iid_name,  GGCX_pop2 = gene_pop2) %>% 
  mutate(rawid = gsub(".B.bed", "", iid_name, perl = T)) %>% 
  select(-iid_name)
...
cyp4f2_a <- A %>% 
  ...
  select(iid_name,  CYP4F2_pop1 = gene_pop1) %>% 
  mutate(rawid = gsub(".A.bed", "", iid_name, perl = T)) %>% 
  select(-iid_name)
...
cyp4f2_b <- B %>% 
  filter(chr ==19 )%>%
  filter(begin <= 15988833 & end >= 16008884) %>% 
  select(iid_name,  CYP4F2_pop2 = gene_pop2) %>% 
  mutate(rawid = gsub(".B.bed", "", iid_name, perl = T)) %>% 
  select(-iid_name)
...
nqo1_a <- A %>% 
  ...
  select(iid_name,  NQO1_pop1 = gene_pop1) %>% 
  mutate(rawid = gsub(".A.bed", "", iid_name, perl = T)) %>% 
  select(-iid_name)
...
nqo1_b <- B %>% 
  filter(chr ==16 )%>%
  filter(begin <= 69743304 & end >= 69760463) %>% 
  select(iid_name,  NQO1_pop2 = gene_pop2) %>% 
  mutate(rawid = gsub(".B.bed", "", iid_name, perl = T)) %>% 
  select(-iid_name)
